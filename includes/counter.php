...
class WP_Post_Views_Counter_Functions {
	...
	public function get_ip_address() {
		...
		if ( isset( $_SERVER['HTTP_CLIENT_IP'] ) ) {
			$client_ip = filter_var( wp_unslash( $_SERVER['HTTP_CLIENT_IP'] ), FILTER_VALIDATE_IP );
		...
		foreach ( $proxy_vars as $var ) {
			if ( isset( $_SERVER[ $var ] ) && ! empty( $_SERVER[ $var ] ) ) {
				$ip = filter_var( wp_unslash( $_SERVER[ $var ] ), FILTER_VALIDATE_IP );
		...
		if ( isset( $_SERVER['REMOTE_ADDR'] ) ) {
			$remote_addr = filter_var( wp_unslash( $_SERVER['REMOTE_ADDR'] ), FILTER_VALIDATE_IP );
	...
	public function ajax_functions() {
		...
		if ( ! wp_verify_nonce( $nonce, 'wp-post-view-nonce' ) ) {
